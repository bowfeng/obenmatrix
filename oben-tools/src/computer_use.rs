...
fn extract_inner(val: &Value, texts: &mut Vec<String>, imgs: &mut Vec<String>) {
    if let Some(obj) = val.as_object() {
        ...
        if let Some(img_data) = obj.get("data").and_then(|v| v.as_str()) {
            if !img_data.is_empty() { imgs.push(img_data.to_string()); }
        } else if let Some(arr) = obj.get("images").and_then(|v| v.as_array()) {
            for img in arr {
                if let Some(s) = img.as_str() { imgs.push(s.to_string()); }
            }
        }
...
fn resolve_target_windows(app: Option<&str>) -> Option<(i64, i64)> {
    let result = call_cua_driver_tool(
        "list_windows",
        ...
        Ok((raw, _)) => {
            if let Ok(obj) = serde_json::from_str::<Value>(&raw) {
                ...
                visible.sort_by_key(|w| {
                    w.get("z_index").and_then(|v| v.as_i64()).unwrap_or(999999)
                });
...
async fn handle_capture(args: &Value, call_id: String) -> Result<ToolResult> {
    ...
    let _max_elements = args.get("max_elements").and_then(|v| v.as_i64()).unwrap_or(100);

    // Resolve target window (list_windows → pid + window_id)
    let (pid, window_id) = resolve_target_windows(app)
...
async fn handle_pointer(args: &Value, action: &str, call_id: String) -> Result<ToolResult> {
    ...
    if let Some(coord) = args.get("coordinate").and_then(|v| v.as_array()) {
        if let (Some(x), Some(y)) = (coord.get(0).and_then(|v| v.as_i64()), coord.get(1).and_then(|v| v.as_i64())) {
...
async fn handle_drag(args: &Value, call_id: String) -> Result<ToolResult> {
    ...
    if let Some(arr) = args.get("from_coordinate").and_then(|v| v.as_array()) {
        if let (Some(x), Some(y)) = (arr.get(0).and_then(|v| v.as_i64()), arr.get(1).and_then(|v| v.as_i64())) {
...
async fn handle_type(args: &Value, call_id: String) -> Result<ToolResult> {
    ...
    if let Some(block) = check_type_safety(text) {
        return Ok(ToolResult { call_id, output: String::new(), error: Some(block) });
    }

    let app = args.get("app").and_then(|v| v.as_str());
    let (pid, _) = resolve_target_windows(app)
        .ok_or_else(|| anyhow::anyhow!("No target window found"))?;

    let args_map = serde_json::Map::from_iter([
        ("text".into(), text.into()),
        ("pid".into(), pid.into()),
    ]);
...
async fn handle_wait(args: &Value, call_id: String) -> Result<ToolResult> {
    let seconds = args.get("seconds").and_then(|v| v.as_f64()).unwrap_or(1.0).max(0.0).min(30.0);
...
async fn handle_list_apps(call_id: String) -> Result<ToolResult> {
    ...
    Ok(ToolResult {
        call_id,
        output,
        error: if text.starts_with("Error:") { Some(text) } else { None },
    })
}
...
async fn handle_focus_app(args: &Value, call_id: String) -> Result<ToolResult> {
    ...
    let output = format!(
        "Targeted window: pid={pid}, window_id={window_id} (app={app})"
    );
    Ok(ToolResult {
        call_id,
        output,
...
fn format_images(images: &[String]) -> String {
    if images.is_empty() { String::new() } else { format!("[{} images]", images.len()) }
}
...
impl SelfRegisteringTool for ComputerUseTool {
    fn tool() -> Tool { make_computer_use_tool() }
    fn handler() -> ToolHandler { make_computer_use_handler() }
}
...
mod tests {
    ...
    async fn test_null_action_returns_error() {
        let r = make_registry();
        let res = r.execute("computer_use", &serde_json::json!({"action": null})).await;
    ...
    async fn test_unknown_action_returns_error() {
        let r = make_registry();
        let res = r.execute("computer_use", &serde_json::json!({"action": "teleport"})).await;
        assert!(res.error.is_some());
