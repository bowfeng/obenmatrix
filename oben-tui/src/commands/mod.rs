...
pub async fn execute_session_rename(app: &mut App, new_name: &str) {
    ...
    match result {
        Ok(old_name) => {
            ...
            if let Some(sessions) = app.panels.get_mut(&PanelId::Sessions)
...
async fn rename_inner(agent: Arc<tokio::sync::Mutex<oben_agent::Agent>>, new_name: &str) -> Result<String, String> {
    ...
    let mut old_title = guard.session_manager_mut().lock().await
        .active_session()
    ...
    if old_title == "unnamed" {
        old_title = guard.session_manager().lock().await
            .active_session()
            .map(|s| s.metadata.title.as_deref().unwrap_or(&s.name).to_string())
            .unwrap_or_else(|| "unnamed".to_string());
    }

    guard.session_manager_mut().lock().await
        .set_title(new_name).map_err(|e| e.to_string())?;
    Ok(old_title)
}
