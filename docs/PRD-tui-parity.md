...
| **Turn controller** | priority-high | ✅ | PR#61 | `oben-tui/src/turn/` | Manages turn lifecycle: start, streaming, idle, interrupted. Tracks tool activity, streaming text, completed tools. Connects to AgentCallbacks (tool_start, tool_complete, stream_delta, reasoning). Real-time UI display with status bar color-coded modes. |
