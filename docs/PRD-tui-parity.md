...
| **Bracket paste detection** | priority-medium | ❌ | — | `useMainApp.ts:72-83` | Detects `\x1b[?2004h` escape; reference toggles bracket paste on stdin. We read raw key events only. |
| **Parse-keypress (full keymap)** | priority-high | ❌ | — | `packages/hermes-ink/src/ink/parse-keypress.ts` | Handles Ctrl+Enter (submit), Alt+Enter (newline), Ctrl+Z (suspend), Cmd+arrows, Ctrl+W, Ctrl+K, Ctrl+A, Ctrl+E, etc. We handle basic arrows + Enter + Ctrl+C + Ctrl+U. |
...
| **System clipboard (native)** | priority-high | ❌ | — | `lib/clipboard.ts` | Platform-agnostic read/write: `pbpaste` (macOS), `powershell` (Windows), `wl-paste` (Wayland), `xclip` (X11). Isolation check for suspicious binary content. |
...
| **Input history (load/append/persist)** | priority-high | ❌ | — | `hooks/useInputHistory.ts`, `lib/history.ts` | Persistent readline-style history file. Up/Down arrow cycling through past inputs. We have no input history at all. |
| **Tab completion (slash + path)** | priority-high | ❌ | — | `hooks/useCompletion.ts` | `complete.slash` and `complete.path` methods. Detects `/` for slash commands, path regex for shell-style completion. Shows completion list with Up/Down navigation. |
...
| **Virtual scrolling (virtual history)** | priority-critical | ❌ | — | `hooks/useVirtualHistory.ts` | Maintains MAX_HISTORY (default 50) transcript items in viewport. Overscans by 20, max mounted 120. Uses height cache for measured row heights, syncs fresh scroll height during streaming. |
...
| **Submission pipeline** | priority-high | ❌ | — | `useSubmission.ts` | Expands paste snippets into text, detects `@file` interpolation, slash command prefix, tab/shift-tab navigation through completions, double- Enter debounce (450ms), session busy error handling. |
