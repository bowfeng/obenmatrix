...
fn test_skill_categories_exist() {
    ...
    assert!(categories.len() >= 20, "Should have at least 20 skill categories, found {}", categories.len());
}
...
fn test_skill_files_exist() {
    ...
    assert!(skill_count >= 20, "Should have at least 20 SKILL.md files, found {}", skill_count);
}

#[test]
fn test_general_skill_exists() {
    let skill_md = Path::new(concat!(env!("CARGO_MANIFEST_DIR"), "/../skills/general/SKILL.md"));
    assert!(skill_md.exists(), "general/SKILL.md should exist");
    
    let content = std::fs::read_to_string(skill_md).unwrap();
    assert!(content.contains("# General"), "Should contain # General header");
    assert!(content.contains("## Capabilities"), "Should contain Capabilities section");
}
