...
pub trait SessionManager: Send + Sync {
    ...
    ///
    /// Used by the Agent/CWM to look up the session that corresponds to a
    /// given key (e.g. "default", a username/channel identifier, etc.).
    ...
    ///
    /// Returns the child session. The CWM tracks which session is active
    /// independently — it no longer lives in SessionManager.
